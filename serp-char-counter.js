...
\*--------------------------------------------------------------------*/
    ...
    mofoCSS.setAttribute('id', 'mofo-stylesheet');
    mofoCSS.setAttribute('rel', 'stylesheet');
    mofoCSS.setAttribute('type', 'text/css');
    mofoCSS.setAttribute('href', mofoStylesheetURL);
...
\*--------------------------------------------------------------------*/
    ...
    jQueryScript.src = jQueryScriptURL;
    document.getElementsByTagName('head')[0].appendChild(jQueryScript);

    // Append YUI script
    var yuiScript = document.createElement('script');
    yuiScript.src = yuiScriptURL;
    document.getElementsByTagName('head')[0].appendChild(yuiScript);

    // If our scripts haven't loaded after this many seconds, just quit.
    var timeout = 10;
    ...
    var poll = function() {
        setTimeout(function() {
            ...
            // jQuery and YUI are loaded...let's proceed.
            ...
            \*--------------------------------------------------------*/
                ...
                // Search results are <li> elements in <ol id="rso">
                var listItems = $('#rso > li.g');
                // Ignore news results
                listItems = listItems.not('#newsbox');
                // Ignore image results
                listItems = listItems.not('#imagebox_bigimages');
                // Ignore video results
                listItems = listItems.not('#videobox');
                // Ignore product results
                listItems = listItems.not('#productbox');
...
//                    }
            ...
            \*--------------------------------------------------------*/
                ...
                for (var m = 0, max = serpItems.length; m < max; m++) {
                    ...
                    jumpContainer.setAttribute('id', 'result_' + (m + 1));
                    jumpContainer.setAttribute('class', 'jump-container');
                    jumpContainer.innerHTML = '<span class="jump-anchor">' + (m + 1) + '</span>';
                    serpItems[m].parentNode.insertBefore(jumpContainer, serpItems[m]);
                }
            ...
            \*--------------------------------------------------------*/
                ...
                mofoHtml +=             '<a href="javascript:void($(\'#mofo-serp-data\').remove());void($(\'#mofo-stylesheet\').remove());void($(\'.jump-anchor\').remove());"><span><b>X</b></span></a>';
                mofoHtml +=         '</div>';
